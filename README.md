...
Commands: `scan`, `login`, `publish`, `sync`, `status`, `providers`, and `logout`. `logout` revokes the server-side device when reachable and rotates the local device identity. Run `tokenlawn --help` for options.
